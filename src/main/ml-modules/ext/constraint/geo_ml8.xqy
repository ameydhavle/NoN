...
declare variable $viz-geo:basediv := 3;
...
{
  let $threshold := ($constraint/search:annotation/*/search:heatmap/@points-threshold/number(.), 1)[1]
  let $add-query := $constraint/search:annotation/search:additional-query/*/cts:query(.)
  let $constraint := viz-geo:_constraintHelper( $constraint, () )
  return
    element search:boxes {
      ...
      return
        if ($freq < 1 or $freq > $threshold) then
          ...
          let $uris := cts:uris((), ("any"), cts:and-query(($query, $add-query, $geo)))
          return
            if (fn:count($uris) eq $freq) then
              for $uri in $uris
              for $p in cts:values(viz-geo:_geospatialReference($constraint), (), ("any"), cts:document-query($uri))
              return
                element search:box {
                  attribute count { 1 },
              ...
              return
                element search:box {
                  ...
                  attribute e { cts:point-longitude($p) },
                  attribute uri { $uri },
                  attribute warn { fn:concat('count mismatch!', fn:count($uris), ' vs. ', $freq) }
                }
    }
};
  ...
  as element(opt:constraint)
{
  ...
  let $options := $constraint/root()
  let $real-options :=
    element { node-name($options) } {
      $options/@*,
      $options/node()[self::opt:constraint/@name != $constraint/@name],
      $real-constraint
    }
  return
    $real-options/*[last()]
